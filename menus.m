  ...
  case "biseccion"
    ...
    while(error != 0)
      try
        ...
        if(isnan(a) || isnan(b))
          msg_error = "Valor de aób no válido";
          disp("Valor de aób no válido");
          error = 1;
  ...
  case "cramer"
    ...
    while(error != 0)
      try
        ...
        if(isempty(matiz_a))
          msg_error = "Matriz no vï¿½lida";
          disp("Matriz no vï¿½lida");
          error = 1;
          questdlg (strcat("ERROR\n",msg_error), "ERROR");
          continue
        endif
    ...
    clc;
              ...
              while(error != 0)
                try
                  ...
                  if(isnan(v) || v < 1)
                    msg_error = "TamaÃ±o del vector no válido";
                    disp("TamaÃ±o del vector no válido");
                    error = 1;
                    questdlg (strcat("ERROR\n",msg_error), "ERROR");
                    continue
                  end
              ...
              while(error != 0)
                try
                  ...
                  for i=1:(v)
                     ...
                     if(isnan(a) || isnan(b))
                       msg_error = "Valor de xóy no válido";
                       disp("Valor de xóy no válido");
                       error = 1;
                       questdlg (strcat("ERROR\n",msg_error), "ERROR");
                       continue
                     end
  ...
  case "interpolacion de newton por diferencias divididas"
    ...
    while(error != 0)
      try
        ...
        if(isnan(a))
          msg_error = "Valor no vï¿½lido";
          disp("Valor no vï¿½lido");
          error = 1;
          questdlg (strcat("ERROR\n",msg_error), "ERROR");
          continue
        endif
  ...
  case "RK4"
    clc;
              ...
              try
                ...
                disp("Error al ejecutar el mÃ©todo rk4")
                return %borrar
              end_try_catch
  ...
  case "punto fijo"
    ...
    while(error != 0)
      try
        ...
        if(isempty(funcion))
          msg_error = "Función no vÃ¡lida";
          disp("Función no vÃ¡lida");
          error = 1;
          questdlg (strcat("ERROR\n",msg_error), "ERROR");
          continue
        endif
